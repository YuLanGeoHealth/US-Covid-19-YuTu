...
async function _dates(d3) {
  const data = await d3.csv(
    "https://raw.githubusercontent.com/YuLan1014/covid/main/both.csv",
    (d) => {
      return d.date;
    }
  );
...
function _map(
  ...
  svg
    .append(legend)
    .attr("transform", "translate(" + 5 * margin + "," + 35 * margin + ")");
...
${format_satscan(data_satscan.get(d.id + " " + dateToString(date)))}`
  ...
  svg
    ...
    .attr(
      "transform",
      "translate(" + (2 * margin + width / 2) + "," + 35 * margin + ")"
    );
  var countyMap = aa
    .append("g")
    ...
    .attr(
      "transform",
      "translate(" + (width / 2 - margin * 2) + "," + margin + ") scale(0.7)"
    )
    .selectAll("path")
    .data(topojson.feature(us, us.objects.counties).features)
    .join("path")
...
async function _data_lisa(d3) {
  return Object.assign(
    new Map(
      await d3.csv(
        "https://raw.githubusercontent.com/YuLan1014/covid/main/2101-04new.csv",
        ({ ID, p, clusterN, date }) => [ID + " " + date, [+p, +clusterN]]
      )
    ),
    { title: ["p value ", "Group "] }
  );
...
async function _data_satscan(d3) {
  return Object.assign(
    new Map(
      await d3.csv(
...
function _format_satscan(data_satscan, labelsRR, x_satscan, labels, y_satscan) {
  return (value) => {
    if (!value) return "N/A";
    let [a, b] = value;
    return `${data_satscan.title[0]}${
      labelsRR[x_satscan(a)] && `${labelsRR[x_satscan(a)]}`
    } 
${data_satscan.title[1]}${labels[y_satscan(b)] && `${labels[y_satscan(b)]}`}`;
  };
}
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("data_lisa")).define("data_lisa", ["d3"], _data_lisa);
  main
    .variable(observer("data_satscan"))
    .define("data_satscan", ["d3"], _data_satscan);
