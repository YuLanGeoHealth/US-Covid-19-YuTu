...
md
`# Animated Bivariate Map & Time Spiral Map of COVID-19 2021 with legend`
)}
...
function _spiral_legend(legend,d3,data_spiral,selected){return(
legend({
...
function _currentState(d3,DOM,topojson,us,color,data,dateToString,date,states,format,legend_map,options,diameter,getStatesNumber,days,GridMap,map_spiral,TimeSpiral,data_spiral,html)
{
  const width = 1200
  const height = 500
  const margin = 10
...
// select and unselect 
  var testdic = {};
    function clicked(e,d) {
      svg.select("text").text(`${d.properties.name}, ${states.get(d.id.slice(0, 2)).name}`);
          d3.selectAll("path")
        .style('stroke', null);
  ...
  //spiralmap legend
// spiralMap.append(legend_spiral)
//     .attr("transform",  "translate("+ (width +margin*10) +","+ "0)");
...
// document.body.append(svg.node());
  ...
  function renderSpiral(d) {   
    ...
    //spiralMap.select(".spiral_legend").remove();
...
function _legend_map(DOM,svg,n,d3,colors,data,labelsRR,labels){return(
() => {
  const k = 16;
  const arrow = DOM.uid();
...
function _selected(currentState){return(
Object.values(currentState)[0]
)}

function _12(data_spiral){return(
data_spiral("California")["cases"]
)}
...
async function _statedata(FileAttachment){return(
await FileAttachment("spiral_allnew.csv").csv()
)}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ...
    ["allcluster.csv", {url: new URL("./files/c3af4157bc9f946d2f26807b269d046fd5aee365321901ef0704b392e46069e0b12241ded8ac79de46b8eff5c73d49198535e789f930052fdb70f63c6d1d5d7f.csv", import.meta.url), mimeType: "text/csv", toString}],
    ["spiral_allnew.csv", {url: new URL("./files/47c3afa5a8ad6a69db1044acff231f6b6d26cda48326c162ac8a814220412642b6505dc6c3dba4481556fa101e4e3d65f46fcc0db3a565534542671aec03caf3.csv", import.meta.url), mimeType: "text/csv", toString}]
  ]);
  ...
  main.variable(observer("viewof spiral_legend")).define("viewof spiral_legend", ["legend","d3","data_spiral","selected"], _spiral_legend);
  main.variable(observer("spiral_legend")).define("spiral_legend", ["Generators", "viewof spiral_legend"], (G, _) => G.input(_));
  main.variable(observer("viewof currentState")).define("viewof currentState", ["d3","DOM","topojson","us","color","data","dateToString","date","states","format","legend_map","options","diameter","getStatesNumber","days","GridMap","map_spiral","TimeSpiral","data_spiral","html"], _currentState);
  main.variable(observer("currentState")).define("currentState", ["Generators", "viewof currentState"], (G, _) => G.input(_));
  main.variable(observer("legend_map")).define("legend_map", ["DOM","svg","n","d3","colors","data","labelsRR","labels"], _legend_map);
  main.variable(observer("selected")).define("selected", ["currentState"], _selected);
