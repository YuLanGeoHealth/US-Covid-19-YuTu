...
function _options(form, html) {
  return form(html`<form style="font-size: 12pt">
    COVID:
    <label for="cases"
      ><input
        name="field"
    ...
    <div style="font-size:10pt;font-style:italic">
      Click a state to lock or switch the highlight. Click the same state again
      or empty space to release the focus.
    </div>
  </form>`);
}
...
function _spiral_legend(legend, d3, data_spiral, selected) {
  return legend({
    color: d3.scaleSequential(
      d3.extent(data_spiral(selected), (d) => +d["cases"]),
      d3[`interpolate${"OrRd"}`]
    ),
    title: selected,
  });
...
${format(data.get(d.id + " " + dateToString(date)))}`
  ...
  function clicked(e, d) {
    ...
    testdic.name = states.get(d.id.slice(0, 2)).name;
    handleClick_bi(d);
  ...
  var spiralMap = svg
    ...
    .attr(
      "transform",
      "translate(" + (width / 2 + margin * 2) + "," + "0) scale(0.9)"
      // "translate(" + (width / 2 + margin * 2) + "," + "0) scale(0.5)"
    )
  ...
  // Mini cartogram
  const mapSize = diameter / 4.2,
    mg = spiralMap.append("g");
  ...
  function handleClick_bi(d) {
    ...
    if (bi_select)
      renderSpiral({ state: bi_select.state, total: bi_select.number });
  }
  ...
  mg.attr(
    "transform",
    `translate(${(diameter - size[0]) / 2.2},${(diameter - size[1]) / 2})`
  );
  ...
  function renderLabels() {
    const label = spiralMap
      ...
      .attr(
        "transform",
        `translate(${(diameter - size[0]) / 2},${(diameter - size[1]) / 2})`
      );
...
function _legend_map(DOM, svg, n, d3, colors, data, labelsRR, labels) {
  return () => {
    const k = 16;
    const arrow = DOM.uid();
...
function _selected(currentState) {
  return Object.values(currentState)[0];
}

function _12(data_spiral) {
  return data_spiral("California")["cases"];
}

function _src(statedata, d3, ms) {
  const td = new Date("2023-03-09");
  // console.log(td);
  ...
  for (let i = data.length - 1; i > 0; i--) {
    const row = data[i];
    if (row.date.getTime() === last) row.lastDay = true;
    else break;
  }
...
function _getStates(src) {
  return (field) => {
    const data = src.filter((d) => d.lastDay);

    const m_month = Math.max.apply(
      null,
      data.map((d) => d.month)
    );
    const m_fips = Math.max.apply(
      null,
      data.map((d) => d.fips)
    );
    ...
    const result = matrix
      .map((d, i) => {
        ...
        for (let m = 0; m < d.length; m++) {
          ...
          row[`${ycode}-${String(mcode).padStart(2, "0")}`] = mdata
            ? mdata[field]
            : 0;
        }
        return row;
...
async function _statedata(FileAttachment) {
  return await FileAttachment("spiral.csv").csv();
}

function _getYesterdayNumbers(getStatesLastNDays) {
  return (field, sorted) => {
    const states = getStatesLastNDays("newcases", 1, false, true),
...
function _getStatesLastNDays(src, mapID) {
  return (field, days, total, code) => {
...
function _getStatesNumber(days, mapID, src) {
  return (field, date) => {
    const td = days,
      mapper = mapID;
    ...
    else data = src.filter((d) => d.date.getTime() === date.getTime());
...
function _TimeSpiral(d3) {
  return class TimeSpiral {
...
function _42(md) {
  return md`
### Appendix of spiral map
  `;
}

async function _mapID(d3, FileAttachment) {
  return new Map(
    d3
      .csvParse(await FileAttachment("state_fips.csv").text(), d3.autoType)
      .map((d) => [d.stname, d.stusps])
  );
}

function _map_spiral() {
  return [
    [0, 0, ""],
    [1, 0, ""],
    [2, 0, ""],
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ...
    [
      ...
      {
        url: new URL(
          "./files/26edb36af0fde039070b2cdaf9a865171e244f093ab0697c0f55b88c81cdc1ed53576b24a3e6c4813d58d732eec847c031838bef446909ac33691ab14035bb9a.csv",
          import.meta.url
        ),
        mimeType: "text/csv",
        toString,
    ...
    [
      "spiral.csv",
      {
        url: new URL("../../data/spiral.csv", import.meta.url),
        mimeType: "text/csv",
        toString,
      },
    ],
  ...
  main
    .variable(observer("viewof options"))
    .define("viewof options", ["form", "html"], _options);
  main
    .variable(observer("options"))
    .define("options", ["Generators", "viewof options"], (G, _) => G.input(_));
  main
  ...
  main
    .variable(observer("viewof spiral_legend"))
    .define(
      "viewof spiral_legend",
      ["legend", "d3", "data_spiral", "selected"],
      _spiral_legend
    );
  main
    .variable(observer("spiral_legend"))
    .define("spiral_legend", ["Generators", "viewof spiral_legend"], (G, _) =>
      G.input(_)
    );
  main
  ...
  main
    .variable(observer("selected"))
    .define("selected", ["currentState"], _selected);
  main.variable(observer()).define(["data_spiral"], _12);
  ...
  main
    .variable(observer("statedata"))
    .define("statedata", ["FileAttachment"], _statedata);
  main
    .variable(observer("getYesterdayNumbers"))
    .define(
      "getYesterdayNumbers",
      ["getStatesLastNDays"],
      _getYesterdayNumbers
    );
  main
    .variable(observer("getStatesLastNDays"))
    .define("getStatesLastNDays", ["src", "mapID"], _getStatesLastNDays);
  main
    .variable(observer("getStatesNumber"))
    .define("getStatesNumber", ["days", "mapID", "src"], _getStatesNumber);
  ...
  main
    .variable(observer("TimeSpiral"))
    .define("TimeSpiral", ["d3"], _TimeSpiral);
  // main.variable(observer()).define(["md"], _47);
  main
    .variable(observer("mapID"))
    .define("mapID", ["d3", "FileAttachment"], _mapID);
  main.variable(observer("map_spiral")).define("map_spiral", _map_spiral);
  main
    .variable(observer("data_spiral"))
    .define("data_spiral", ["src"], _data_spiral);
  main.variable(observer("today")).define("today", _today);
  main
    .variable(observer("yesterday"))
    .define("yesterday", ["today"], _yesterday);
