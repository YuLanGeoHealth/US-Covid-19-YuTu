...
md
`# Zipcode Level| Animated Bivariate Map & TimeChart of COVID-19 2021`
)}
...
function _mapvar(Inputs){return(
Inputs.radio(["Cases", "Deaths"], {value: "Cases", label: "Display"})
)}
 ...
 // create SVG element
  ...
  var countyMap = bim.append("g")
   //.attr("transform","translate("+margin*25+","+margin+") scale(0.7)")
    .selectAll("path")
    .data(topojson.feature(us, us.objects.wizip).features)
    .join("path")
    ...
    .on("mouseout", function(d) {
      ...
      .on('click', clicked)
    .append("title")
    .text(d => `${d.properties.GEOID},${format(data.get(d.properties.GEOID+" "+ dateToString(date)))}`);
  
  bim.append("path")
      .datum(topojson.mesh(us, us.objects.wizip, (a, b) => a !== b))
     // .attr("transform","translate("+margin*25+","+margin+") scale(0.7)")
      .attr("fill", "none")
      .attr("stroke", "white")
...
// select and unselect 
  ...
  var testdic = {};
    function clicked(e,d) {
      svg.select("text").text(`${d.properties.GEOID}`);
 if(!testdic.hasOwnProperty(d.properties.GEOID)){
     selected = this;
     //selects.push(d.id);
   testdic[d.properties.GEOID]=d.properties.GEOID
 ...
 //else if(selects.includes(d.id)){
   ...
   else if(testdic.hasOwnProperty(d.properties.GEOID)){
         selected = this;
    d3.select(selected).style('stroke', null);
    selected = undefined;
...
{
  const data = await d3.csv(  "https://raw.githubusercontent.com/YuLan1014/covid/main/wi_cluster_2021120120220228.csv", d => {
    return d.date;
  });
  return [...new Set(data)];
}
...
async function _data(mapvar,d3)
{
  var datasel ={};
  switch(mapvar){
    case "Cases":
     var datasel = Object.assign(new Map(
  await d3.csv(  "https://raw.githubusercontent.com/YuLan1014/covid/main/wi_cluster_2021120120220228.csv", ({ID,RR_C,RR_L,date}) => [(ID+ " "+date), [+RR_C, +RR_L]])), 
                     {title: ["RR of cluster", "RR of zipcode"]}
                    )
      return datasel
        break;
          case "Deaths":
      var datasel = Object.assign(new Map(
  await d3.csv(  "https://raw.githubusercontent.com/YuLan1014/covid/main/wi_death_cluster_2021120120220228.csv", ({ID,RR_C,RR_L,date}) => [(ID+ " "+date), [+RR_C, +RR_L]])), 
                     {title: ["RR of cluster", "RR of zipcode"]}
                    )
      return datasel
        break;
...
function _all_data(mapvar,d3)
{
  var datasel ={};
  switch(mapvar){
    case "Cases":
     var datasel = d3.csv(
    "https://raw.githubusercontent.com/YuLan1014/covid/main/wi_rr.csv",
       d3.autoType
                    )
      return datasel
        break;
          case "Deaths":
      var datasel = d3.csv(
    "https://raw.githubusercontent.com/YuLan1014/covid/main/wi_death_rr.csv",
      d3.autoType
      )
      return datasel
        break;
...
function _all_data2(mapvar,d3)
{
  var datasel ={};
  switch(mapvar){
    case "Cases":
     var datasel = d3.csv(
     "https://raw.githubusercontent.com/YuLan1014/covid/main/wi_cluster_2021120120220228.csv",
       d3.autoType
                    )
      return datasel
        break;
          case "Deaths":
      var datasel = d3.csv(
    "https://raw.githubusercontent.com/YuLan1014/covid/main/wi_death_cluster_2021120120220228.csv",
       d3.autoType)
      return datasel
        break;
  }
...
function _24(currentState){return(
Object.keys(currentState)
)}

function _25(currentState){return(
currentState
)}
...
function _stop(d3,raw_data){return(
...
function _40(us){return(
us.objects
)}

function _states(us){return(
new Map(us.objects.wizip.geometries.map(d => [d.id, d.properties.GEOID]))
)}

function _us(FileAttachment){return(
FileAttachment("wiziptopo.json").json()
)}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["wiziptopo.json", {url: new URL("./files/3f4bceb9dbce3819fdb4668b09baa807496b48cd0e44472ccb7897abd5f4d105700e48782c85dfb95f65651b8985564016edd3642aaded37ee633c05b105683a.json", import.meta.url), mimeType: "application/json", toString}]
  ]);
  ...
  main.variable(observer("viewof mapvar")).define("viewof mapvar", ["Inputs"], _mapvar);
  main.variable(observer("mapvar")).define("mapvar", ["Generators", "viewof mapvar"], (G, _) => G.input(_));
  ...
  main.variable(observer("data")).define("data", ["mapvar","d3"], _data);
  ...
  main.variable(observer("all_data")).define("all_data", ["mapvar","d3"], _all_data);
  main.variable(observer("all_data2")).define("all_data2", ["mapvar","d3"], _all_data2);
  ...
  main.variable(observer("stop")).define("stop", ["d3","raw_data"], _stop);
  ...
  main.variable(observer()).define(["us"], _40);
