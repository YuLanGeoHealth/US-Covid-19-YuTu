...
function _1(md) {
  return md`
# 3D Relative Risk of U.S. Lower 48 with Plotly with basemap using RR_C and RR_L 2020/09/01~2022/06/30
  `;
}

function _start(date) {
  return date({ value: "2022-01-01" });
}

function _end(date) {
  return date({ value: "2022-06-30" });
}
...
function _mapvar(Inputs) {
  return Inputs.radio(["rr_c", "rr_l"], { value: "rr_c", label: "Display" });
}
  ...
  const zipScatter = [
    ...
    },
    {
      ...
      hovertemplate:
        ...
        "%{text[3]}<br>" +
        "<extra></extra>",
      text: zipData.map((d) => [d.rr_c, d.rr_l, d.county_full, d.state_id]),
      type: "scatter3d",
    },
  ]; // zipScatter
  const layout = {
...
function _population(mapvar, zipData) {
  switch (mapvar) {
    case "rr_c":
      return zipData.map((d) => d.rr_c);
      break;
    case "rr_l":
...
async function _zipData(FileAttachment, mapvar, MinRR, start, end) {
  // const data = await fetchData("https://raw.githubusercontent.com/YuLan1014/covid/main/test.csv");
  // const data = await fetchData("https://raw.githubusercontent.com/YuLan1014/covid/main/rr3dnew.csv");
  const datanew = await FileAttachment("3d.csv").csv();
  // const data = await fetchData("https://raw.githubusercontent.com/YuLan1014/covid/main/allcluster.csv");

  //const lower48Abr = Object.keys(lower48);
  switch (mapvar) {
    case "rr_c":
      var filtered = datanew.filter((d) => d.rr_c > MinRR);
      break;
    case "rr_l":
      var filtered = datanew.filter((d) => d.rr_l > MinRR);
      break;
  }
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    [
      "3d.csv",
      {
        url: new URL("../../data/3d.csv", import.meta.url),
        mimeType: "text/csv",
        toString,
      },
    ],
  ...
  main
    .variable(observer("viewof mapvar"))
    .define("viewof mapvar", ["Inputs"], _mapvar);
  main
    .variable(observer("mapvar"))
    .define("mapvar", ["Generators", "viewof mapvar"], (G, _) => G.input(_));
  main
  ...
  main
    ...
    .define("population", ["mapvar", "zipData"], _population);
  main
  ...
  main
    ...
    .define(
      "zipData",
      ["FileAttachment", "mapvar", "MinRR", "start", "end"],
      _zipData
    );
