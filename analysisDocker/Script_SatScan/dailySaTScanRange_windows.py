...
directory = "c:\\work\\dissertation\\satscan_data\\results\\"
savedir = "/mnt/batch/"
# %%
## extract data according to time ##
df = pd.read_csv('/mnt/sqldata/cases.csv')
fipslist = df.fips.unique().tolist()
...
folder = "/mnt/batch/" +filename+'_'
prmdir = directory + filename+'_'
print(prmdir)
print(folder)
# %%
# output files for satscan: cas and geo
df_geo = df_satscan[['fips','lat','long']]
df_geo = df_geo.set_index('fips')

df_cas = df_satscan[['fips','case_avg7','collect_date']]
df_cas['case_avg7'] = df_cas['case_avg7'].astype(int)
df_cas = df_cas.set_index('fips')

df_pop = pd.read_csv('/mnt/sqldata/pop.csv')
df_pop['d'] = 'unspecified'
df_pop = df_pop.reindex(['fips','d','pop_total'], axis=1)
df_pop = df_pop.loc[df_pop['fips'].isin(fipslist)]
...
windowdir = "C:\\work\\dissertation\\satscan_data\\batch\\"+filename+'_'
case = windowdir+'case.csv'
geo = windowdir+'geo.csv'
pop = windowdir+'pop.csv' 
input_text=f"[Input]\n \
;case data filename\n \
CaseFile={case}\n \
